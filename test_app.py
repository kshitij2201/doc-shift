...
def get_top_web_contents(query, num_results=5):
    try:
...
def check_similarity(user_text, content):
    return round(difflib.SequenceMatcher(None, user_text, content).ratio() * 100, 2)
...
def check_plagiarism():
    try:
        ...
        query = user_text[:100]
        web_contents = get_top_web_contents(query)
        results = []
        for url, content in web_contents:
            score = check_similarity(user_text, content)
            if score > 10:
                results.append({'url': url, 'score': score})
        results.sort(key=lambda x: x['score'], reverse=True)

        # Save results to a file for logging
